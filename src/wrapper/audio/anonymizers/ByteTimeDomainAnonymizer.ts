...
export default class ByteTimeDomainAnonymizer extends AbstractAnonymizer<AnalyserNode,void> {
    onFake(orig, __this, _arguments) {
        let targetBuff = _arguments[0];
        if (TypeGuards.isUint8Array(targetBuff)) {
            const anonymized = this.processor.addNoiseToByteTimeDomainData((buffView) => {
